<section class="footer-section" id="footer">

    <div class="foot container" style="padding-block:50px; font-size: 130%;">
        <br>
        <div class="row">
            <div class="col-sm-4">
                <div class="row">

                    <div class="col-4"> <img src="images/logo.png" alt="" class="logoFooter" 
                            style="margin-right:0px;"></div>
                    <div class="col">
                        <h2 class="sampingLogo" style="color:#7D4B22;"><strong>
                                Gereja Katolik Santo Yusup Jember</strong></h2>
                    </div>
                </div>
                {{-- <br> --}}
                <p class="mt-4 textFooter"><img src="images/phone.png" height="20" alt=""> ( 0331 ) 486 478</p>
                {{-- <br> --}}
                <p class="mt-4 textFooter"><img src="images/mail.png" height="20" alt=""> styusupjember@gmail.com</p>
                {{-- <br> --}}
                <p class="mt-4 textFooter"><img src="images/youtube.png" height="20" alt=""> Santo Yusup</p>
            </div>
            {{-- <div class="col-sm"></div>
            <div class="col-sm"></div>
            <div class="col-sm"></div> --}}
            <div class="col-sm-4 textFooter" style="margin-top:30px">
                <strong> Rekening Kolekte :</strong>
                <br>
                BCA Jember.
                BPGDA RK Gereja Santo Yosef
                A/C : 024 852 1000
                <br>
                <br>
                <strong>Rekening Sumbangan untuk Paroki :</strong>
                <br>
                Bank Mandiri Jember.
                BPGDA Roma Katolik Gereja Santo Yosep
                A/C : 143 00 04724 934
            </div>
            <div class="col-sm-4 textFooter" style="margin-top:30px">
                <strong> Rekening Pembangunan :</strong>
                <br>
                Bank Mandiri Jember
                143 00 0559199 3
                <br>
                <br>
                <br>
                <strong> Rekening Pastoran untuk Intensi Misa :</strong>
                <br>
                Bank CIMB Niaga Jember
                Herman Yoseph Suyatna, Yohanes Suparno
                A/C : 7033 6368 5000
            </div>
        </div>
        <p class="text-center mt-4 textFooter"><strong>Jl. R.A. Kartini No.26, Tembaan, Kepatihan, Kec. Kaliwates, Kabupaten
                Jember, Jawa
                Timur 68131</strong></p>
    </div>
    <div class="p-3 text-center textFooter" style="background-color: #603B1B;  color: #F6F3C4;">© 2022. Gereja Katolik Paroki Santo
        Yusup Jember. All Right
        Reserved.</div>
</section>
